...
class Leinwand < TkCanvas
  ...
  def anzahl_objekte()
    puts "Anzahl Objekte #{@figuren_graphiken.size()}" 
  end
  ...
  def zuruecksetzen()
    @figuren_graphiken.each(){|key,val|
      self.delete(val.id)
    }
    @figuren_graphiken.clear()
  end
  ...
  def zeichne_rechteck(rechteck) 
    ...
    rechteck_graphik =TkcRectangle.new(self, x1,y1,x2,y2,:fill => rechteck.fuell_farbe(), :outline => rechteck.farbe)
  ...
  def farbe_aendern(figur)
      if @figuren_graphiken.include?(figur)
        ...
        figur_graphik[:fill]= figur.fuell_farbe
