...
class Rechteck
  ...
  #NEU: den Rechteck langsam horizontal bewegen
  def langsam_horizontal_bewegen(entfernung)
      ...
      if(sichtbar?) 
        ...
        x_delta = delta
        y_delta = 0
        Leinwand.gib_einzige_instanz().bewege(self,absolute_entfernung,x_delta,y_delta)
      end 
      @obere_linke_ecke = @obere_linke_ecke + Point.new(entfernung,0)
  end
  
  #NEU: Rechteck bewegen, basierend auf der vorhandenen bewege Methode
  def bewegen(wiederholungen, x_delta, y_delta, wiederholen_nach, starten_nach)
  
      
    Leinwand.gib_einzige_instanz().bewege(self, wiederholungen,x_delta,y_delta, wiederholen_nach=100, starten_nach=0 )
    
    @obere_linke_ecke = @obere_linke_ecke + Point.new(x_delta*wiederholungen,y_delta*wiederholungen)
  end
